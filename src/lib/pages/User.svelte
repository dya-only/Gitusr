...
<main>
  ...
  <div class="under w-screen flex justify-center items-center flex-col">
    <!-- Followers -->
    <div class="flwrs-card p-10 rounded-l-2xl rounded-r-2xl drop-shadow-2xl bg-[#161B22] max-h-[500px] overflow-auto scrollbar scrollbar-thumb-neutral-700 scrollbar-thumb-rounded-full scrollbar-thin flex justify-center items-center flex-wrap max-w-[400px]">
      { #each flws as flw }
      <!-- svelte-ignore a11y-click-events-have-key-events -->
      <img class="flw-img m-2 rounded-2xl w-12 cursor-pointer transition ease-out hover:brightness-50" src={ flw.avatar } alt="" on:click={ () => { window.open(`http://github.com/${flw.name}`, '_blank') } } />
      { /each }
